...
less.inject = function(lessCss) {
	new(less.Parser)({
		...
		var css = tree.toCSS();
			...
			document.insertBefore(css, document.childNodes[document.childNodes.length-1]);
	});
};
	...
	for (var name in variables) {
		...
		for (var sheetId in sheets) {
			...
			if (sheetUrl) { // If a sheet url was defined only process sheets that have a matching url
				var _sheet = less.sheetmap[sheetId];
			...
			{
				sheetUpdates[sheetId] = {sheet: less.sheetmap[sheetId], variables: {}};
